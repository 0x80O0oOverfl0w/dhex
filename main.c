...
int parsecommandlineoptions(int argc,char** argv,tInt64* baseaddr1,tInt64* baseaddr2,tInt64* cursorpos1,tInt64* cursorpos2,tBool* diffmode,int* filename1,int* filename2,tBool* keyboardsetupreq,char* configfile,tSearch* search1,tBool* gosearch1,tSearch* search2,tBool* gosearch2,tCorrelation* correlation,tBool* gocorr)
{
	int filenamecnt=0;
	int i;
	int retval=RETOK;
...
//	tBuffer buf2;
	...
	if (parsecommandlineoptions(argc,argv,&baseaddr1,&baseaddr2,&cursorpos1,&cursorpos2,&diffmode,&filename1,&filename2,&keyboardsetupreq,configfile,&search1,&gosearch1,&search2,&gosearch2,&correlation,&gocorr)!=RETOK)
	{
		if (output)
		{
...
#undef	MOVEMENTDEFINE
			...
			if ((ch==KEYF1) || (ch==':' && windowfield==0))
			{
				if(!diffmode)
				{
					if (gotomask(output,&cursorpos1,buf1->baseaddr)==RETOK)
					{
						firstpos1=cursorpos1;	
					}
				} else {
					if (gotomaskdiff(output,&cursorpos1,&cursorpos2,buf1->baseaddr,buf2->baseaddr)==RETOK)
					{
						firstpos1=cursorpos1;
						firstpos2=cursorpos2;
					}
			...
			if ((ch==KEYF2 || ((ch=='/' || ch=='?') && (windowfield==0))))	
			{
				if(!diffmode)
				{
					...
					if (ch=='/')	search1.forwardnotbackward=1;	// / means forward
					if (ch=='?')	search1.forwardnotbackward=0;	// ? means backward
					searchmaskdiff(output, &search1, buf1, buf2, &cursorpos1, &cursorpos2);
					firstpos1=cursorpos1;
					firstpos2=cursorpos2;
				}
			}
